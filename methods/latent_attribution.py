...
class LatentAttributionXAI(BaseXAI):
    ...
    def _gradient_based_attribution(
        self, input_tensor: Tensor, latent_dim: int
    ) -> Tensor:
        ...
        with torch.enable_grad():
            ...
            if (
                ...
                and output.__class__.__name__ == "ModelOutput"
            ):
                ...
                if "mu" in output:
                    ...
                    raise ValueError(
                        "Could not extract latent representation from model output dict"
                    )
                ...
                if hasattr(self.model, "get_latent"):
                    ...
                    raise ValueError(
                        "Model output is a tensor but no encoder method found"
                    )
            else:
                raise ValueError(
    ...
    def _perturbation_based_attribution(
        self, input_tensor: Tensor, latent_dim: int
    ) -> Tensor:
        ...
        with torch.no_grad():
            ...
            # Extract latent representation based on model output format
            if (
                hasattr(baseline_output, "__class__")
                and baseline_output.__class__.__name__ == "ModelOutput"
            ):
                # ModelOutput from refrakt_core.schema.model_output
                ...
                hasattr(baseline_output, "latent")
                and baseline_output.latent is not None
            ):
                # ModelOutput object with latent attribute
                baseline_latent = baseline_output.latent
                ...
                hasattr(baseline_output, "embeddings")
                and baseline_output.embeddings is not None
            ):
                # ModelOutput object with embeddings attribute
                baseline_latent = baseline_output.embeddings
            ...
            for i in range(input_tensor.shape[1]):
                for j in range(input_tensor.shape[2]):
                    for k in range(input_tensor.shape[3]):
                        ...
                        # Extract latent representation based on model output format
                        if (
                            hasattr(perturbed_output, "__class__")
                            and perturbed_output.__class__.__name__ == "ModelOutput"
                        ):
                            # ModelOutput from refrakt_core.schema.model_output
                            ...
                            hasattr(perturbed_output, "mu")
                            and perturbed_output.mu is not None
                        ):
                            # ModelOutput object with mu attribute
                            perturbed_latent = perturbed_output.mu
                        elif (
                            hasattr(perturbed_output, "latent")
                            and perturbed_output.latent is not None
                        ):
                            # ModelOutput object with latent attribute
                            perturbed_latent = perturbed_output.latent
                        elif (
                            hasattr(perturbed_output, "z")
                            and perturbed_output.z is not None
                        ):
                            # ModelOutput object with z attribute
                            perturbed_latent = perturbed_output.z
                        elif (
                            hasattr(perturbed_output, "embeddings")
                            and perturbed_output.embeddings is not None
                        ):
                            # ModelOutput object with embeddings attribute
                            perturbed_latent = perturbed_output.embeddings
                            ...
                            # We need to get the latent from the encoder
                            if hasattr(self.model, "get_latent"):
                                perturbed_latent = self.model.get_latent(
                                    perturbed_input
                                )
                            elif hasattr(self.model, "encode"):
